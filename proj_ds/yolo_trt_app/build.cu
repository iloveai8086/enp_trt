...
int main(int argc, char **argv)
{
    if (argc < 5)
    {
        std::cerr << "用法: ./build [onnx_file_path] [use_int8] [calib_dir] [calib_list_file]" << std::endl;
        return -1;
    }
    // 命令行获取onnx文件路径、校准数据集路径、校准数据集列表文件
    char *onnx_file_path = argv[1];
    auto use_int8 = std::stoi(argv[2]);
    char *calib_dir = argv[3];
    char *calib_list_file = argv[4];
    ...
    if (!builder->platformHasFastInt8() or !use_int8)
    {
