...
void cuda_preprocess(
  ...
  CUDA_CHECK(cudaMemcpy(img_buffer_device, img_buffer_host, img_size, cudaMemcpyHostToDevice));
...
void process_input(cv::Mat& input_img, float* input_device_buffer) {
  ...
  cv::Mat resize_img;
  cv::resize(rgb, resize_img, cv::Size(kInputW, kInputH));
  cuda_preprocess_init(kInputH*kInputW);
  cuda_preprocess(
    resize_img.ptr(), kInputW, kInputH,
